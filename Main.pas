...
type
  TForm1 = class(TForm)
    ...
    procedure UpdateKeikakujwmstData;
...
procedure TForm1.UpdateKeikakujwmstData;
var
  UpdateQuery: TUniQuery; // ประกาศตัวแปรสำหรับ Query ใหม่
begin
  UpdateQuery := TUniQuery.Create(nil);
  try
    // เชื่อมต่อฐานข้อมูลด้วย UniConnection ที่อยู่บนฟอร์มของคุณ
    UpdateQuery.Connection := UniConnection;
    ...
    UpdateQuery.SQL.Text :=
      ...
      + ' WHERE t.SETNO <> 0 '                                        + #13
      + '   AND t.JKBN = 4 '                                          + #13
      + '   AND t.INPEXENAME = ''MW10IMP100.exe'' '                   + #13
      + '   AND EXISTS '                                              + #13
      + ' ( '                                                         + #13
      + '     SELECT 1 '                                              + #13
      + '     FROM KEIKAKUJWMST s '                                   + #13
    ...
    try
      ...
      begin
        // จัดการ Error
        ShowMessage('Error updating JYMDS/JYMDE: ' + E.Message);
      end;
    end;

  finally
    UpdateQuery.Free; // คืนค่าหน่วยความจำเสมอ
  end;
end;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          // 2026/08/13: On Complete (JKBN=4) always use the full-update path so
          // that JYMDS/JYMDE and start dates are refreshed to the CSV values for
          // every SETNO (including split sets SETNO<>0 that were skipped during
          // the previous Continue import and were still holding the plan date).
          if InsertQuery.IsEmpty or (JKBNValue = '4') then
          begin
            InsertQuery.SQL.Text :=
